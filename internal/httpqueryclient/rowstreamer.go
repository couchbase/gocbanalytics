...
func (s *rowStreamer) begin() error {
	if s.state != rowStreamStateStart {
		return errors.New("unexpected parsing state during begin") //nolint:err113
	}

	// Read the opening { for the result
	t, err := s.decoder.Token()
	if err != nil {
		return newObfuscateErrorWrapper("failed to read opening brace", err)
	}

	if delim, ok := t.(json.Delim); !ok || delim != '{' {
		return errors.New("expected an opening brace for the result") //nolint:err113
	}
	...
	for {
		...
		t, err = s.decoder.Token()
		if err != nil {
			return newObfuscateErrorWrapper("failed to read attribute name", err)
		}

		key, keyOk := t.(string)
		if !keyOk {
			return errors.New("expected an object property name") //nolint:err113
		}
		...
		if key == s.rowsAttrib {
			...
			t, err = s.decoder.Token()
			if err != nil {
				return newObfuscateErrorWrapper("failed to read opening bracket for rows", err)
			}
			...
			if delim, ok := t.(json.Delim); !ok || delim != '[' {
				return errors.New("expected an opening bracket for the rows") //nolint:err113
			}
		...
		if err != nil {
			return newObfuscateErrorWrapper("failed to read attribute value", err)
		}
...
func (s *rowStreamer) readRow() (json.RawMessage, error) {
	if s.state < rowStreamStateRows {
		return nil, errors.New("unexpected parsing state during readRow") //nolint:err113
	}
	...
	if err != nil {
		return nil, newObfuscateErrorWrapper("failed to decode row", err)
	}
...
func (s *rowStreamer) end() error {
	if s.state < rowStreamStatePostRows {
		return errors.New("unexpected parsing state during end") //nolint:err113
	}
	...
	t, err := s.decoder.Token()
	if err != nil {
		return newObfuscateErrorWrapper("failed to read ending bracket", err)
	}

	if delim, ok := t.(json.Delim); !ok || delim != ']' {
		return errors.New("expected an ending bracket for the rows") //nolint:err113
	}
...
func (s *rowStreamer) Finalize() (json.RawMessage, error) {
	...
	if err != nil {
		return nil, newObfuscateErrorWrapper("failed to reconstruct metadata", err)
	}
